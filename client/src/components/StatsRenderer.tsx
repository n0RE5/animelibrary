...
const StatsRenderer: React.FC<StatsRendererProps> = ({animeStats}) => {
    return (
        <div>
            {animeStats.map((stats: AnimeStatsI, index) => {
                return <div key={index} className={classes.StatsRenderer_flex}>
                            <div key={'left'+index} className={classes.StatsRenderer_left}>{stats.stat_name}</div>
                            <div key={'right'+index} className={classes.StatsRenderer_right}>
